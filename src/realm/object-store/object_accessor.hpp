...
auto get_mixed(Context& ctx, const object_store::Collection& collection, const PathElement& path, Mixed value)
{
    if (value.is_type(type_Dictionary)) {
...
template <typename T>
std::optional<CollectionType> collection_type(T)
{
    return {};
}
...
auto unbox(Context& ctx, T&& value, U&, CreatePolicy policy)
{
    return ctx.template unbox<U>(value, policy);
}

template <typename Context, typename T>
auto unbox(Context& ctx, T&& value, Obj& old_obj, CreatePolicy policy)
{
    return ctx.template unbox<Obj>(value, policy, old_obj.get_key());
}
}
...
auto List::get(Context& ctx, size_t row_ndx) const
{
    if (m_type == PropertyType::Mixed) {
        return _impl::get_mixed(ctx, *this, row_ndx, get<Mixed>(row_ndx));
    }
...
void List::insert(Context& ctx, size_t list_ndx, T&& value, CreatePolicy policy)
{
    ...
    dispatch([&](auto t) {
        auto new_val = ctx.template unbox<std::decay_t<decltype(*t)>>(value, policy);
...
void List::set(Context& ctx, size_t list_ndx, T&& value, CreatePolicy policy)
{
    ...
    dispatch([&](auto t) {
        ...
        if (policy.diff) {
            ...
            auto new_value = _impl::unbox(ctx, std::forward<T>(value), old_value, policy);
            if (_impl::compare(old_value, new_value))
                return;
        }
...
{
    if (m_type == PropertyType::Mixed) {
        return _impl::get_mixed(ctx, *this, key, get<Mixed>(key));
    }
    return dispatch([&](auto t) {
