...
bool RealmCoordinator::open_db()
{
    ...
    if (m_config.sync_config) {
        ...
        if (auto provider = m_config.sync_config->user->provider().lock()) {
            m_sync_session = provider->sync_manager()->get_existing_session(m_config.path);
        }
        if (m_sync_session) {
...
void RealmCoordinator::init_external_helpers()
{
    ...
    if (m_config.sync_config && !m_sync_session) {
        if (auto provider = m_config.sync_config->user->provider().lock()) {
            m_sync_session = provider->sync_manager()->get_session(m_db, m_config);
        }
        else {
