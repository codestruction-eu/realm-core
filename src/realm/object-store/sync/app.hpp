...
private:
    ...
    std::string make_sync_route(std::optional<std::string> ws_host_url = util::none) REQUIRES(m_route_mutex);
    ...
    std::weak_ptr<User> m_current_user GUARDED_BY(m_user_mutex);
