...
{
    // it feels like there's probably a race condition with this being called
    // at the wrong time as a sync session is being established on another thread
    ...
    if (sync_manager) {
        sync_manager->update_sessions_for(*this, SyncUser::State::LoggedIn, SyncUser::State::Removed, {});
    }
}
...
void User::request_log_out(util::UniqueFunction<void(util::Optional<app::AppError>)>&& completion)
{
    if (auto app = this->app()) {
        app->log_out(shared_from_this(), std::move(completion));
    }
}
