...
void PushClient::register_device(const std::string& registration_token, const std::shared_ptr<AppUser>& sync_user,
...
void PushClient::deregister_device(const std::shared_ptr<AppUser>& sync_user,
                                   util::UniqueFunction<void(util::Optional<AppError>)>&& completion)
{
    auto push_route = util::format("/app/%1/push/providers/%2/registration", m_app_id, m_service_name);
