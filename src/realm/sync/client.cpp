...
void SessionWrapper::handle_pending_client_reset_acknowledgement()
{
    ...
    async_wait_for(true, true, [self = util::bind_ptr(this), pending_reset = *pending_reset](Status status) {
        ...
        if (!cur_pending_reset) {
            logger.debug(util::LogCategory::reset, "cycle detection tracker already removed", pending_reset.time);
            return;
        }
        if (cur_pending_reset->mode != pending_reset.mode || cur_pending_reset->action != pending_reset.action ||
            ...
            logger.debug(util::LogCategory::reset, "Removing cycle detection tracker.");
        }
