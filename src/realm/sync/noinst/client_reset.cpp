                                      ...
                                      util::Logger& logger)
{
    if (auto previous_reset = has_pending_reset(wt)) {
        ...
        if (action != previous_reset->action) {
            ...
            logger.info(util::LogCategory::reset,
                        ...
                        recovery_allowed ? "recoverable" : "non-recoverable", mode, action);
            if (error) {
    ...
    track_reset(wt, mode, recovery_allowed, action, error);
    return mode;
}
                               ...
                               util::FunctionRef<void(int64_t)> on_flx_version_complete)
{
    ...
    auto actual_mode = reset_precheck_guard(*wt_local, reset_config.mode, reset_config.recovery_allowed,
