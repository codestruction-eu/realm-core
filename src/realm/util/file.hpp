...
public:
    ...
    size_t read(uint64_t pos, char* data, size_t size);
    static size_t read_static(FileDesc fd, uint64_t pos, char* data, size_t size);
    ...
    void write(uint64_t pos, const char* data, size_t size);
    static void write_static(FileDesc fd, uint64_t pos, const char* data, size_t size);

    // Tells current file pointer of fd
    uint64_t get_file_pos();

    /// Calls write(s.data(), s.size()).
    void write(uint64_t pos, std::string_view s)
    {
    ...
    /// Calls read(data, N).
    template <size_t N>
    size_t read(uint64_t pos, char (&data)[N])
    {
    ...
    /// Calls write(data(), N).
    template <size_t N>
    void write(uint64_t pos, const char (&data)[N])
    {
        write(pos, data, N);
    }
...
inline void File::Streambuf::flush()
{
    size_t n = pptr() - pbase();
    if (n > 0) {
        uint64_t pos = m_file.get_file_pos();
