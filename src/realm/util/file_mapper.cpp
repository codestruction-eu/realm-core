...
void* mmap(const FileAttributes& file, size_t size, uint64_t offset, std::unique_ptr<EncryptedFileMapping>& mapping)
{
    ...
    if (file.encryption) {
        ...
        size += offset - page_start;
        size = round_up_to_page_size(size);
        void* addr = mmap_anon(size);
        mapping = file.encryption->add_mapping(page_start, addr, size, file.access); // FIXME: leaks if throws
...
EncryptedFile::EncryptedFile(const char* key)
    : cryptor(key)
{
}
#endif // REALM_ENABLE_ENCRYPTION
