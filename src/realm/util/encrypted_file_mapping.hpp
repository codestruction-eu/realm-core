...
class EncryptedFile {
public:
    EncryptedFile(const char* key);

    void set_fd(FileDesc fd)
    {
        this->fd = fd;
    }
...
public:
    ...
    // Does not call fsync
    void flush() noexcept REQUIRES(!m_file.mutex);
    ...
    bool contains_block(size_t block_in_file) const noexcept REQUIRES(m_file.mutex);
...
private:
    ...
    size_t m_first_block GUARDED_BY(m_file.mutex);
    ...
    std::vector<BlockState> m_block_state GUARDED_BY(m_file.mutex);
    // little helpers:
    void clear(BlockState& ps, int p)
    {
        ps = BlockState(ps & ~p);
    }
    bool is_not(BlockState& ps, int p)
    {
        return (ps & p) == 0;
    }
    bool is(BlockState& ps, int p)
    {
        return (ps & p) != 0;
    }
    void set(BlockState& ps, int p)
    {
        ps = BlockState(ps | p);
    }
    ...
    char* block_addr(size_t local_ndx) const noexcept REQUIRES(m_file.mutex);
    File::SizeType block_pos(size_t local_ndx) const noexcept REQUIRES(m_file.mutex);
    void mark_outdated(size_t local_ndx) noexcept REQUIRES(m_file.mutex);
    bool copy_up_to_date_block(size_t local_ndx) noexcept REQUIRES(m_file.mutex);
    bool check_possibly_stale_block(size_t local_ndx) noexcept REQUIRES(m_file.mutex);
    void refresh_block(size_t local_ndx, bool to_modify) REQUIRES(m_file.mutex);
    void write_and_update_all(size_t local_ndx, uint16_t offset, uint16_t size) noexcept REQUIRES(m_file.mutex);
    void validate_block(size_t local_ndx) noexcept REQUIRES(m_file.mutex);
    void validate() noexcept REQUIRES(m_file.mutex);
    void do_flush() noexcept REQUIRES(m_file.mutex);
...
{
#if REALM_DEBUG
    auto state_name = [](const BlockState& s) -> std::string {
        if (s == BlockState::Clean) {
            return "Clean";
        }
        std::string state = "{";
        if (s & BlockState::UpToDate) {
            state += "UpToDate";
        }
        if (s & BlockState::StaleIV) {
            state += "StaleIV";
        }
        if (s & BlockState::Writable) {
            state += "Writable";
        }
        if (s & BlockState::Dirty) {
            state += "Dirty";
        }
        state += "}";
        return state;
    ...
    for (BlockState& s : m_block_state) {
        if (!page_states.empty()) {
            page_states += ", ";
        }
        page_states += state_name(s);
    }
    return util::format("%1 blocks from %2 to %3: %4", m_block_state.size(), m_first_block,
                        m_block_state.size() + m_first_block, page_states);
#else
    return "";
#endif // REALM_DEBUG
}
