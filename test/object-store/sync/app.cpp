...
TEST_CASE("app: sync integration", "[sync][pbs][app][baas]") {
    ...
    public:
                                    ...
                                    util::UniqueFunction<void(const Response&)>&& completion) override
        {
            ...
            SynchronousTestTransport::send_request_to_server(request, [&](const Response& response) mutable {
                Response modifiable_response = response;
                if (response_hook) {
                    response_hook(request, modifiable_response);
                }
                completion(modifiable_response);
            });
        }
...

    {
        ...
        std::string base_file_path = util::make_temp_dir();
        ...
        app_config.base_file_path = base_file_path;
        app_config.metadata_mode = AppConfig::MetadataMode::NoEncryption;

        // initialize app and sync client
        auto redir_app = app::App::get_app(app::App::CacheMode::Disabled, app_config);
        ...
        SECTION("Test too many redirects") {
            ...
            redir_app->log_in_with_credentials(
                ...
                [&](std::shared_ptr<User> user, util::Optional<app::AppError> error) {
                    REQUIRE(!user);
                    REQUIRE(error);
                    REQUIRE(error->is_client_error());
    ...
    SECTION("Test app redirect with no metadata") {
        ...
        set_app_config_defaults(app_config, redir_transport);
        app_config.base_file_path = base_file_path;
        app_config.metadata_mode = realm::app::AppConfig::MetadataMode::InMemory;
...
TEST_CASE("app: base_url", "[sync][app][base_url]") {
    ...
    SECTION("Verify new sync session updates location") {
        ...
        socket_provider->endpoint_verify_func = [use_ssl, expected_host, expected_port](sync::WebSocketEndpoint& ep) {
...
TEST_CASE("app: app destroyed during token refresh", "[sync][app][user][token]") {
    ...
    {
        auto [cur_user_promise, cur_user_future] = util::make_promise_future<std::shared_ptr<User>>();
        app->log_in_with_credentials(AppCredentials::anonymous(),
                                     [promise = std::move(cur_user_promise)](std::shared_ptr<User> user,
...
TEST_CASE("app: app cannot get deallocated during log in", "[sync][app]") {
...

    {
        ...
        oas.app()->log_in_with_credentials(AppCredentials::anonymous(),
                                           [promise = std::move(cur_user_promise)](
                                               std::shared_ptr<User> user, util::Optional<AppError> error) mutable {
                                               REQUIRE_FALSE(error);
                                               promise.emplace_value(std::move(user));
                                           });
    }
...
TEST_CASE("app: user logs out while profile is fetched", "[sync][app][user]") {
    ...
    app->log_in_with_credentials(
        custom_credentials,
        [promise = std::move(cur_user_promise)](std::shared_ptr<User> user, util::Optional<AppError> error) mutable {
            REQUIRE_FALSE(error);
            promise.emplace_value(std::move(user));
        });
